...
struct ListDetailView: View {
    ...
    private func recognizeTransactionFromPhoto(data: Data) async {
        ...
        do {
            let result = try await callAIVisionAPI(imageData: data)
    ...
    private func callAIVisionAPI(imageData: Data) async throws -> RecognizedTransactionData {
        ...
        let categoryNames = expenseCategories.map(\.name).joined(separator: ", ")
    ...
    private func compressImage(_ data: Data, maxDimension: CGFloat = 1536) -> Data? {
...
private struct RecognizedTransactionData: Codable {
    let title: String
    let amount: Double
